...
- レポート Markdown を stdin で渡し、エージェントの出力（Markdown、`### 分析サマリー` / `### コスト削減提案` / `### 利用パターンの気づき` の3節）を「内訳」の直後（セッション一覧の手前）にマージします（Markdown 出力では `## AI 分析（<agent>）` セクション、HTML 出力では同トーンのカード）
